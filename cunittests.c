...
void self_test_1(void)
{
  ...
  retval = beargit_init();
  CU_ASSERT(0==retval);
  FILE* fileone = fopen("1.txt", "w");
  fclose(fileone);
  retval = beargit_add("1.txt");
  CU_ASSERT(0==retval);
  run_commit(&commit_list, "GO BEARS!1");

  FILE* filetwo = fopen("2.txt", "w");
  fclose(filetwo);
  retval = beargit_add("2.txt");
  CU_ASSERT(0==retval);
  run_commit(&commit_list, "GO BEARS!2");

  retval = beargit_rm("1.txt");
  CU_ASSERT(0 == retval);
  run_commit(&commit_list, "GO BEARS!3");

  retval = beargit_checkout("Thor", 1);
  CU_ASSERT(0 == retval);
  FILE* filefour = fopen("4.txt", "w");
  fclose(filefour);
  retval = beargit_add("4.txt");
  CU_ASSERT(0==retval);
  run_commit(&commit_list, "GO BEARS!4");
  ...
  FILE* filethree = fopen("3.txt", "w");
  fclose(filethree);
  retval = beargit_add("3.txt");
  CU_ASSERT(0==retval);
  run_commit(&commit_list, "GO BEARS!IRONMAN");

  retval = beargit_branch();
  CU_ASSERT(0 == retval);

  char linetwo[LINE_SIZE];

  CU_ASSERT_PTR_NOT_NULL(fgets(linetwo, LINE_SIZE, fstdout));
  strtok(linetwo, "\n");
  CU_ASSERT(!strcmp(linetwo, "* master"));

  CU_ASSERT_PTR_NOT_NULL(fgets(linetwo, LINE_SIZE, fstdout));
  strtok(linetwo, "\n");
  CU_ASSERT(!strcmp(linetwo, "  Thor"));
...
void self_test_2(void)
{
  ...
  strtok(line, "\n");
  CU_ASSERT(!strcmp(line, "ERROR: File 1.txt not tracked"));
...
int cunittester()
{
   ...
   /* Add a suite to the registry. Self-written test 1 */
   ...
   if (NULL == CU_add_test(pSuite3, "Self-test 1", self_test_1))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }

   /* Add a suite to the registry. Self-written test 2 */
   ...
   if (NULL == CU_add_test(pSuite4, "Self-test 2", self_test_2))
   {
      CU_cleanup_registry();
      return CU_get_error();
   }
